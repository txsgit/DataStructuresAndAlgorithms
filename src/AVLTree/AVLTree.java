...
public class AVLTree {
     ...
     */
    public  boolean isBalanced_Solution(AVLTreeNode root) {
        if (root == null)
            return true;
        ...
        return result<=1&&isBalanced_Solution(root.leftNode)&&isBalanced_Solution(root.rightNode);
    }
...
//        tree.insert(5);
        ...
        tree.insert(15);
        //不是平衡二叉树
        AVLTreeNode root=new AVLTreeNode(16);
        AVLTreeNode rootL=new AVLTreeNode(3);
        root.leftNode=rootL;
        AVLTreeNode rootLR=new AVLTreeNode(7);
        rootL.rightNode=rootLR;

        if(tree.isBalanced_Solution(root))
        {
